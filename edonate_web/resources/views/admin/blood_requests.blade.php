...
<main class="request-content">
    ...
    <section class="request-panel">
        ...
        <div class="admin-pagination admin-pagination--js" aria-label="Table pagination">
            <span class="admin-pagination__info" id="requestMeta">Showing 0 to 0 of 0 entries</span>
            <nav class="admin-pagination__links" id="requestPaginationLinks" aria-label="Pagination links"></nav>
        </div>
    </section>
</main>
...
<script>
(function(){'use strict';
const urls={data:@json(route('admin.blood-requests.data')),store:@json(route('admin.blood-requests.store'))},csrf=document.querySelector('meta[name="csrf-token"]').content,rows=document.getElementById('requestRows'),paginationLinks=document.getElementById('requestPaginationLinks'),requestMeta=document.getElementById('requestMeta');let page=1,timer=null;
...
async function load(){rows.innerHTML='<tr><td colspan="11" class="text-center text-muted py-4">Loading...</td></tr>';try{const data=await req(`${urls.data}?${params()}`),s=data.summary||{},meta=data.meta||{};page=Number(meta.current_page||page);document.getElementById('requestOpen').textContent=s.open??0;document.getElementById('requestEmergency').textContent=s.emergency??0;document.getElementById('requestFulfilled').textContent=s.fulfilled??0;document.getElementById('requestCancelled').textContent=s.cancelled??0;if(window.eDonateAdminPagination)window.eDonateAdminPagination.render(paginationLinks,meta,null,{infoElement:requestMeta});rows.innerHTML=(data.data||[]).length?(data.data||[]).map(r=>`<tr><td><strong>${esc(r.request_reference)}</strong></td><td>${esc(r.facility_name)}</td><td>${esc(head(r.request_type))}</td><td>${esc(r.needed_blood_type)}</td><td>${esc(r.required_donors)}<br><small class="text-muted">${esc(r.specific_match_required)} exact</small></td><td>${esc(r.notified_count)}</td><td>${esc(r.interested_count)}</td><td><span class="request-badge request-badge--${esc(r.urgency)}">${esc(head(r.urgency))}</span></td><td><span class="request-badge request-badge--${esc(r.status)}">${esc(head(r.status))}</span></td><td>${r.created_at?esc(new Date(r.created_at).toLocaleString()):''}</td><td><a class="btn btn-sm btn-outline-danger" href="${esc(r.show_url)}">View</a></td></tr>`).join(''):'<tr><td colspan="11" class="text-center text-muted py-4">No blood requests found.</td></tr>'}catch(e){rows.innerHTML='<tr><td colspan="11" class="text-center text-danger py-4">Could not load blood requests.</td></tr>';if(requestMeta)requestMeta.textContent='Showing 0 to 0 of 0 entries';if(paginationLinks)paginationLinks.innerHTML=''}}
paginationLinks.addEventListener('click',(event)=>{const button=event.target.closest('button[data-page]');if(!button)return;page=Number(button.dataset.page||1);load();});['requestSearch','requestStatus','requestUrgency','requestBloodType'].forEach(id=>document.getElementById(id).addEventListener(id==='requestSearch'?'input':'change',()=>{clearTimeout(timer);timer=setTimeout(()=>{page=1;load()},250)}));document.getElementById('requestClear').onclick=()=>{['requestSearch','requestStatus','requestUrgency','requestBloodType'].forEach(id=>document.getElementById(id).value='');page=1;load()};
const modalElement=document.getElementById('requestModal'),form=document.getElementById('requestForm'),errors=document.getElementById('requestErrors');let modal=null;const getModal=()=>{if(!modal&&modalElement&&window.bootstrap&&window.bootstrap.Modal)modal=window.bootstrap.Modal.getOrCreateInstance(modalElement);return modal;};document.getElementById('createRequestButton').onclick=()=>{form.reset();errors.classList.add('d-none');const modalInstance=getModal();if(modalInstance)modalInstance.show()};form.onsubmit=async e=>{e.preventDefault();const payload=Object.fromEntries(new FormData(form));payload.allow_other_blood_types=payload.allow_other_blood_types==='1';if(!payload.expires_at)payload.expires_at=null;document.getElementById('requestSave').disabled=true;try{await req(urls.store,{method:'POST',body:JSON.stringify(payload)});const modalInstance=getModal();if(modalInstance)modalInstance.hide();load()}catch(err){errors.textContent=err.message||Object.values(err.errors||{}).flat().join(' ');errors.classList.remove('d-none')}finally{document.getElementById('requestSave').disabled=false}};load();
}());
</script>
@endpush

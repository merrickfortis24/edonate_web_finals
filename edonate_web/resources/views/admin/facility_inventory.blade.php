...
@push('admin_head')
<style>
    .inventory-content{padding:24px 32px 40px}.inventory-panel{background:#fff;border:1px solid rgba(0,0,0,.12);border-radius:8px;box-shadow:0 2px 8px rgba(0,0,0,.06);margin-bottom:18px;padding:18px}.inventory-grid{display:grid;gap:14px;grid-template-columns:repeat(4,minmax(0,1fr))}.inventory-field label{display:block;font-size:12px;font-weight:600;margin-bottom:5px}.inventory-status{border-radius:999px;display:inline-block;font-size:11px;font-weight:700;padding:4px 8px}.inventory-status--available{background:#dff3e4;color:#17642c}.inventory-status--low{background:#fff0bd;color:#725500}.inventory-status--out_of_stock{background:#f8d8d8;color:#8a1616}.inventory-table-wrap{overflow-x:auto}.inventory-table{min-width:820px}.inventory-table td,.inventory-table th{font-size:13px;vertical-align:middle}@media(max-width:900px){.inventory-grid{grid-template-columns:repeat(2,1fr)}}@media(max-width:600px){.inventory-content{padding:16px}.inventory-grid{grid-template-columns:1fr}}
</style>
@endpush
